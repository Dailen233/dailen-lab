<script setup>
import { ref, onMounted, computed, watch, reactive, toRefs, provide, readonly } from "vue";
import CandidateDraft from "./components/CandidateDraft.vue";
...
const receivedDraft = ref(null);
const showDraftA = ref(true);
const projectName = ref("JobInsight 学习项目");

provide("projectName", readonly(projectName));

// watch(
//     searchKeyword, 
//     (newValue, oldValue) => {
//         console.log("原来的关键词：", oldValue);
//         console.log("现在的关键词：", newValue);
//     },
//     // { immediate: true }
// );
...
watch(
    ...
    (newValue, oldValue) => {
        console.log("处理后的关键词变化：", oldValue, "→", newValue);
    }
);
...
const filteredCandidates = computed(() => {
    ...
    return candidates.value.filter(candidate => {
        return candidate.name.includes(keyword)
            || candidate.target_job.includes(keyword);
    });
});

function toggleDraftA() {
    showDraftA.value = !showDraftA.value;
}

function handleDraftSubmit(draft) {
    receivedDraft.value = draft;

    console.log("父组件收到草稿：", draft);
}
    ...
    <main class="page">
        ...
        <div class="form-row">
            ...
            <input
                ...
                type="text"
                placeholder="输入姓名或目标岗位"
            >
        </div>

        <p>
            已加载 {{ candidates.length }} 人，
            当前显示 {{ filteredCandidates.length }} 人。
        </p>
        ...
        <!-- <CandidateDraft title="新增候选人草稿" /> -->

        <!-- <CandidateDraft title="草稿 A" />
        <CandidateDraft title="草稿 B" />    -->
        ...
        <button type="button" @click="toggleDraftA">
            {{ showDraftA ? "关闭草稿 A" : "打开草稿 A" }}
        </button>

        <CandidateDraft 
            v-show="showDraftA"
            title="草稿 A"
            @submit-draft="handleDraftSubmit"
        />
        ...
        <section>
            ...
            <p v-if="receivedDraft">
                姓名：{{ receivedDraft.name }}，
                目标岗位：{{ receivedDraft.target_job }}
            </p>
